...
func solve() {
	X1 := customIo.GetNextInt()
	Y1 := customIo.GetNextInt()
	X2 := customIo.GetNextInt()
	Y2 := customIo.GetNextInt()
	DX := X1 - X2
	DY := Y1 - Y2
	Distance := math.Sqrt(float64(DX*DX + DY*DY))

	ans := "No"
	if Distance == 2 || Distance == 4 || Distance == math.Sqrt(2) || Distance == math.Sqrt(10) || Distance == math.Sqrt(18) || Distance == math.Sqrt(20) {
		ans = "Yes"
	}
