...
func solve() {
	...
	Shop[n] = Distance
	for i := 1; i < n; i++ {
		Shop[i] = customIo.GetNextInt()
	}
	sort.Ints(Shop)
	sum := 0.0
	for j := 0; j < m; j++ {
		d := customIo.GetNextInt()
		index := sort.SearchInts(Shop, d)
		if index <= len(Shop) {
			sum += math.Min(math.Abs(float64(d-Shop[index-1])), math.Abs(float64(Shop[index]-d)))
		}
	}
	customIo.Print(sum)
}
