...
func solve() {
	...
	q := customIo.GetNextInt()

	graph := make([][]int, n)
	for i := 0; i < n-1; i++ {
		from := customIo.GetNextInt() - 1
...
//深さ優先探索
func dfs(index, pa int, graph [][]int) {
	for _, nextV := range graph[index] {
		if pa != nextV {
			Counter[nextV] += Counter[index]
			dfs(nextV, index, graph)
		}
	}
}
