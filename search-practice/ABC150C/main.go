...
func solve() {
	...
	for {
		count++
		if equalSlice(sortNum, P) {
			pNum = count
		}
		if equalSlice(sortNum, Q) {
			qNum = count
		}
		if !nextPermutation(sort.IntSlice(sortNum)) {
			break
		}
	}
	customIo.Print(math.Abs(float64(pNum - qNum)))
